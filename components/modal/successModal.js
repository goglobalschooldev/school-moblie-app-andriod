...
export default function SuccessModal({ modalVisible, setModalVisible, data }) {
  ...
  const handleCreate = async () => {
    setVisible(!visible);
    await createPickingUp({
      variables: {
        newPickingUp: {
          studentId: stuData?.getStudentCardByStudentID?._id,
          picked: true,
        },
      },
      update(_, result) {
        setPickResult(result?.data?.createPickingUp);
      },
    });
  };
